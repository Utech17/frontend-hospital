<template>
  <div class="side-menu" :class="{ 'side-menu-minimized': !isOpen }" :style="{ width: currentWidth }">
...
<style scoped>
  ...
  /* ensure label text doesn't wrap and shows ellipsis when long */
  .nav-link span,
  .submenu-link span {
...
/* Adjust visuals when side menu is minimized */
.side-menu-minimized {
  width: 80px;
}
...
/* Center icons and hide text labels when minimized */
.side-menu-minimized .nav-link {
  justify-content: center;
  padding: 0.6rem 0.4rem;
}

.side-menu-minimized .nav-link img {
  margin: 0 !important;
  width: 20px;
  height: 20px;
}

.side-menu-minimized .nav-link span {
  display: none !important; /* hide labels */
}

/* Do not show nested submenus when minimized */
.side-menu-minimized .submenu,
.side-menu-minimized .nested-submenu {
  display: none !important;
}

/* Make the toggle button smaller when minimized and center it */
.side-menu-minimized .d-flex > button.btn {
  padding: 6px;
  width: 34px;
  height: 34px;
}

/* Ensure logo area doesn't produce overflow when hidden */
.side-menu img[alt="Toggle"] {
  max-width: 100%;
  height: auto;
}
</style>